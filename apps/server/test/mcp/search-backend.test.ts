...
//
// 注意：web_search 永远不开浏览器（PlaywrightSearchBackend 已删），后端只有 DuckDuckGo/BingApi/endpoint。
...
async function importBackend() {
  return await import('../../src/mcp/search-backend.js');
}
...
describe('resolveSearchBackend() 引擎映射（同步）', () => {
  ...
  it('默认（未配 env）→ DuckDuckGoSearchBackend（零依赖，不开浏览器）', async () => {
...
describe('FallbackSearchBackend 顺序尝试', () => {
  ...
  it('全部失败 → 抛聚合错误（含每条后端失败原因 + 安装指引）', async () => {
    ...
    await expect(chain.search('q', 5)).rejects.toThrow(
      /全部不可用[\s\S]*chromium not found[\s\S]*DDG unreachable[\s\S]*YANZHI_BING_API_KEY/,
    );
  });
