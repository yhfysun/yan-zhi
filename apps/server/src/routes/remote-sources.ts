...
router.get('/', (req: Request, res: Response) => {
  ...
  const rows = db.prepare(
    'SELECT id, name, type, base_url, auth_type, enabled, created_at FROM remote_marketplace WHERE user_id = ? ORDER BY created_at DESC',
  ).all(userId);
  res.json({ data: rows });
});
...
router.post('/', (req: Request, res: Response) => {
  const userId = req.user!.userId;
  const { name, type, baseUrl, authType, authConfig } = req.body || {};
  if (!name || !type || !baseUrl) { res.status(400).json({ error: 'name, type, baseUrl 为必填项' }); return; }
  if (!['skill', 'agent', 'tool'].includes(type)) { res.status(400).json({ error: 'type 必须为 skill, agent 或 tool' }); return; }

  const id = uuid();
  const now = Date.now();
  db.prepare(
    `INSERT INTO remote_marketplace (id, user_id, name, type, base_url, auth_type, auth_config_enc, enabled, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?)`,
  ).run(id, userId, name, type, baseUrl, authType || 'none', authConfig ? JSON.stringify(authConfig) : null, now);
  res.json({ data: db.prepare('SELECT id, name, type, base_url, auth_type, enabled, created_at FROM remote_marketplace WHERE id = ?').get(id) });
});

router.patch('/:id', (req: Request, res: Response) => {
  const userId = req.user!.userId; const sid = req.params.id;
  const e = db.prepare('SELECT * FROM remote_marketplace WHERE id = ? AND user_id = ?').get(sid, userId) as any;
  ...
  res.json({ data: db.prepare('SELECT id, name, type, base_url, auth_type, enabled, created_at FROM remote_marketplace WHERE id = ?').get(sid) });
});

router.delete('/:id', (req: Request, res: Response) => {
  const userId = req.user!.userId; const sid = req.params.id;
  if (!db.prepare('SELECT id FROM remote_marketplace WHERE id = ? AND user_id = ?').get(sid, userId)) {
...
router.post('/:id/test', async (req: Request, res: Response) => {
  const s = db.prepare('SELECT * FROM remote_marketplace WHERE id = ? AND user_id = ?').get(req.params.id, req.user!.userId) as any;
  ...
  try {
    ...
    if (s.auth_config_enc) {
      ...
      else if (s.auth_type === 'basic' && c.username) headers['Authorization'] = 'Basic ' + Buffer.from(`${c.username}:${c.password || ''}`).toString('base64');
    }
