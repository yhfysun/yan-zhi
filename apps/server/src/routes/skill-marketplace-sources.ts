...
router.get('/:id/skills', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const resp = await fetch(`${s.base_url.replace(/\/$/, '')}/api/marketplace/skills?page=${page}&pageSize=${pageSize}`, { headers: authHeaders(s) });
...
router.post('/:id/install', async (req: Request, res: Response) => {
  ...
  try {
    const resp = await fetch(`${s.base_url.replace(/\/$/, '')}/api/marketplace/skills/${encodeURIComponent(skillId)}`, { headers: authHeaders(s) });
