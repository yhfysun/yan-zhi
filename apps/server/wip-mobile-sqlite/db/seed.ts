...
try {
  const convCols = await driver.all(`SELECT name FROM pragma_table_info('conversation')`, []) as Array<{ name: string }>;
  if (!convCols.some((c) => c.name === 'builtin_tool_ids_json')) {
    await driver.exec(`ALTER TABLE conversation ADD COLUMN builtin_tool_ids_json TEXT DEFAULT '[]'`);
  }
} catch {}
...
-- 不能放在此处（旧库 conversation 表无 space_id 列会导致整个 await driver.exec() 失败，连带 conversation_file 等后续表也无法创建）。
-- 该索引已移至迁移代码块之后创建。
CREATE INDEX IF NOT EXISTS idx_space_user ON space(user_id);
  ...
  -- 注意：conversation_file 的索引必须在该表创建之后（见下方），否则 await driver.exec() 会因表不存在而中断。
...
for (const col of ['alias', 'remark']) {
  try { await driver.exec(`ALTER TABLE mcp_tool ADD COLUMN ${col} TEXT`); } catch {}
}
// enabled 列：前端 store 一直在读写它，但建表语句里从未创建过，老库会直接报 no such column
try { await driver.exec('ALTER TABLE mcp_tool ADD COLUMN enabled INTEGER NOT NULL DEFAULT 1'); } catch {}

// 迁移 mcp_server 表（绑定凭证：auth_credential_id 指向 mcp_credential.id）
try { await driver.exec('ALTER TABLE mcp_server ADD COLUMN auth_credential_id TEXT'); } catch {}

// 迁移平台/模型表（添加内置标记）
for (const table of ['platform', 'model']) {
  try { await driver.exec(`ALTER TABLE ${table} ADD COLUMN is_builtin INTEGER NOT NULL DEFAULT 0`); } catch {}
}

// 迁移平台表（添加停顿时间范围配置，用于多 Token 轮询的请求节流）
try { await driver.exec('ALTER TABLE platform ADD COLUMN pause_min_ms INTEGER NOT NULL DEFAULT 0'); } catch {}
try { await driver.exec('ALTER TABLE platform ADD COLUMN pause_max_ms INTEGER NOT NULL DEFAULT 0'); } catch {}
...
try {
  const rows = await driver.all("SELECT id, user_id, api_key_enc FROM platform WHERE api_key_enc IS NOT NULL AND api_key_enc != ''", []) as any[];
...
try { await driver.exec('ALTER TABLE conversation ADD COLUMN space_id TEXT'); } catch {}
// 迁移完成后才能创建引用 space_id 的索引（旧库迁移场景）
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_conversation_space ON conversation(space_id)'); } catch {}

// 迁移 message 表（添加 system_prompt_snapshot 列 + 子智能体归属列）
try { await driver.exec('ALTER TABLE message ADD COLUMN system_prompt_snapshot TEXT'); } catch {}
for (const col of ['parent_tool_call_id TEXT', 'sub_agent_id TEXT', 'sub_agent_name TEXT', 'sub_agent_depth INTEGER']) {
  ...
  try { await driver.exec(`ALTER TABLE message ADD COLUMN ${col}`); } catch {}
  void name;
}

// 迁移 skill 表（添加商城相关字段）
for (const col of ['source', 'remote_source_id']) {
  try { await driver.exec(`ALTER TABLE skill ADD COLUMN ${col} TEXT`); } catch {}
}
try { await driver.exec('ALTER TABLE skill ADD COLUMN is_public INTEGER NOT NULL DEFAULT 0'); } catch {}

// 迁移 agent 表（添加商城相关字段，如果 agent 表存在）
try {
  for (const col of ['source', 'remote_source_id']) {
    try { await driver.exec(`ALTER TABLE agent ADD COLUMN ${col} TEXT`); } catch {}
  }
  try { await driver.exec('ALTER TABLE agent ADD COLUMN is_public INTEGER NOT NULL DEFAULT 0'); } catch {}
} catch {}

// 迁移 agent 表（Harness 模式 + 挂载字段）
try {
  try { await driver.exec("ALTER TABLE agent ADD COLUMN type TEXT NOT NULL DEFAULT 'harness'"); } catch {}
  for (const col of ['builtin_tool_ids', 'custom_tool_ids', 'mcp_tool_mounts', 'skill_ids', 'sub_agent_ids']) {
    try { await driver.exec(`ALTER TABLE agent ADD COLUMN ${col} TEXT`); } catch {}
  }
  // E4: 内置智能体标记（如 pageAgent）
  try { await driver.exec('ALTER TABLE agent ADD COLUMN is_builtin INTEGER NOT NULL DEFAULT 0'); } catch {}
  // 智能体归属用户（guest=未登录/桌面本地；放开登录后按真实用户隔离）
  try { await driver.exec('ALTER TABLE agent ADD COLUMN user_id TEXT'); } catch {}
  // 智能体分类：main=主智能体（会话可选中）/ sub=子智能体（仅供其他智能体引用委派）
  try { await driver.exec("ALTER TABLE agent ADD COLUMN agent_kind TEXT NOT NULL DEFAULT 'main'"); } catch {}
} catch {}
...
try {
  const defaultExists = await driver.get("SELECT 1 FROM agent WHERE id = 'a_default_assistant'", []);
  if (defaultExists) {
    await driver.exec("UPDATE conversation SET agent_id = 'a_default_assistant' WHERE agent_id IS NULL OR agent_id = ''");
  }
} catch {}

// 迁移：custom_tool / agent 新增 installs 计数列（商城安装计数）
for (const t of ['custom_tool', 'agent']) {
  try { await driver.exec(`ALTER TABLE ${t} ADD COLUMN installs INTEGER NOT NULL DEFAULT 0`); } catch {}
}

// 迁移：custom_tool 新增 category 分类列（自定义工具归类；旧数据回填为「其他」）
try { await driver.exec("ALTER TABLE custom_tool ADD COLUMN category TEXT NOT NULL DEFAULT '其他'"); } catch {}

// 迁移：model 新增 description 列（模型描述，供 list_models 工具与前端展示）
try { await driver.exec('ALTER TABLE model ADD COLUMN description TEXT'); } catch {}
...
`);
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_chat_peer_status ON chat_peer(status, last_seen_at DESC)'); } catch {}
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_chat_message_peers ON chat_message(to_peer_id, created_at)'); } catch {}
...
`);
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_im_connector_user ON im_connector(user_id, provider)'); } catch {}
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_im_inbound_provider ON im_inbound_event(provider, created_at)'); } catch {}
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_im_conv_map_conv ON im_conversation_map(conversation_id)'); } catch {}
...
`);
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_kb_base_user ON knowledge_base(user_id)'); } catch {}
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_kb_doc_base ON knowledge_doc(base_id)'); } catch {}
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_kb_chunk_base ON knowledge_chunk(base_id, doc_id)'); } catch {}

// 知识库向量检索：给 knowledge_chunk 加 embedding BLOB 列（存 Float32Array 的 buffer）
try { await driver.exec('ALTER TABLE knowledge_chunk ADD COLUMN embedding BLOB'); } catch {}
...
try {
  const cols = await driver.all(`SELECT name FROM pragma_table_info('knowledge_base')`, []) as Array<{ name: string }>;
  if (!cols.some((c) => c.name === 'visibility')) {
    await driver.exec(`ALTER TABLE knowledge_base ADD COLUMN visibility TEXT NOT NULL DEFAULT 'private'`);
  }
} catch {}
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_kb_visibility ON knowledge_base(user_id, visibility)'); } catch {}

// 预置内置 guest 用户：未登录（web/桌面）统一用它作为访客身份访问服务端知识库。
// 该用户不可登录（占位哈希），其创建的知识库强制 public，供所有用户共享浏览。
try {
  const guest = await driver.get("SELECT id FROM user WHERE username = 'guest'", []);
  if (!guest) {
    await driver.run("INSERT INTO user (id, username, email, password_hash, created_at) VALUES ('guest', 'guest', NULL, '!guest-internal-account', 0)", []);
  }
} catch {}
...
for (const a of seedAgents) {
  try {
    const has = await driver.get('SELECT id FROM agent WHERE id = ?', [a.id as string]);
    if (has) {
      ...
      if (a.force_sync && overwriteEnabled) {
        await driver.run('UPDATE agent SET is_public = 1, is_builtin = ?, user_id = COALESCE(user_id, ?), name = ?, builtin_tool_ids = ?, sub_agent_ids = ?, skill_ids = ?, type = ?, system_prompt = ?, config_json = ?, agent_kind = ? WHERE id = ?', [a.is_builtin || 0, 'guest', a.name as string, a.builtin_tool_ids as string, (a.sub_agent_ids as string) || '[]', (a.skill_ids as string) || '[]', a.type as string, a.system_prompt as string, (a.config_json as string) || null, (a.agent_kind as string) || 'main', a.id as string]);
      } else {
        await driver.run('UPDATE agent SET is_public = 1, user_id = COALESCE(user_id, ?), builtin_tool_ids = ?, sub_agent_ids = ?, skill_ids = ?, type = ?, system_prompt = COALESCE(system_prompt, ?) WHERE id = ?', ['guest', a.builtin_tool_ids as string, (a.sub_agent_ids as string) || '[]', (a.skill_ids as string) || '[]', a.type as string, a.system_prompt as string, a.id as string]);
      }
    } else {
      await driver.run('INSERT INTO agent (id, user_id, name, description, system_prompt, type, builtin_tool_ids, sub_agent_ids, skill_ids, is_default, is_public, is_builtin, version, config_json, agent_kind, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, 1, ?, ?, ?, ?)', [a.id, 'guest', a.name, a.description, a.system_prompt as string, a.type, a.builtin_tool_ids, (a.sub_agent_ids as string) || '[]', (a.skill_ids as string) || '[]', a.is_default || 0, a.is_builtin || 0, (a.config_json as string) || null, (a.agent_kind as string) || 'main', Date.now(), Date.now()]);
    }
  } catch {}
}
...
try {
  await driver.run("UPDATE agent SET name = '日常办公助手' WHERE id = 'a_default_assistant' AND name IN ('AI 助手', 'AI助手')", []);
} catch {}
...
try {
  ...
  const defRow = await driver.get("SELECT system_prompt FROM agent WHERE id = 'a_default_assistant'", []) as { system_prompt?: string | null } | undefined;
  if (defRow && !(defRow.system_prompt || '').includes(MARKER)) {
    const next = (defRow.system_prompt || DEFAULT_AGENT_SYSTEM_PROMPT) + '\n\n' + WEB_QUERY_PROMPT_BLOCK;
    await driver.run("UPDATE agent SET system_prompt = ? WHERE id = 'a_default_assistant'", [next]);
  }
} catch {}
...
try {
  ...
  if (row) {
    ...
    if (Array.isArray(ids) && !ids.includes(DATA_AGENT_ID)) {
      await driver.run("UPDATE agent SET sub_agent_ids = ? WHERE id = 'a_default_assistant'", [JSON.stringify([...ids, DATA_AGENT_ID])]);
    }
...
try {
  ...
  const row = await driver.get("SELECT system_prompt FROM agent WHERE id = 'a_default_assistant'", []) as { system_prompt?: string | null } | undefined;
  if (row?.system_prompt && row.system_prompt.includes(OLD_LINE)) {
    await driver.run("UPDATE agent SET system_prompt = ? WHERE id = 'a_default_assistant'", [row.system_prompt.replace(OLD_LINE, NEW_LINES)]);
  }
} catch {}
...
for (const t of seedCustomTools) {
  try {
    const has = await driver.get('SELECT id FROM custom_tool WHERE name = ?', [t.name]);
    if (has) continue;
    ...
    await driver.run(`INSERT INTO custom_tool (id, user_id, name, description, input_schema_json, output_schema_json,
       runtime, entry, code, dependencies_json, timeout, env_json, enabled, source, is_public, installs, updated_at, created_at)
       VALUES (?,?,?,?,?,NULL,'node',?,?,?,?,NULL,1,'local',0,0,?,?)`, [id, 'guest', t.name, t.description, JSON.stringify(t.inputSchema), t.entry, t.code, JSON.stringify([]), 5000, now, now]);
  } catch (e: unknown) {
...
try {
  ...
  const hasSkill = await driver.get('SELECT id FROM skill WHERE id = ?', [skillId]);
  if (!hasSkill) {
    await driver.run('INSERT INTO skill (id, user_id, name, description, triggers_json, body, category, author, enabled, installs, source, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [skillId, 'guest', '网站自动化任务',
    ...
    if (overwriteEnabled) {
      await driver.run("UPDATE skill SET body = ?, description = ?, triggers_json = ?, source = 'builtin' WHERE id = ?", [`# 网站自动化任务\n\n用内置 pageAgent 驱动真实浏览器（预览面板）完成网站操作任务。pageAgent 仅挂载六件套工具：browser_navigate / browser_type / browser_click / browser_scroll / browser_get_page_content / ask_user，其余浏览器工具（get_dom/fill_form/search/翻页等）不可用。配合「定时任务」可每日自动执行。\n\n## 流程\n1. 委派 pageAgent：call_agent { agentId: "a_builtin_page_agent", input: "<任务描述：目标站点 + 要执行的操作/要提取的信息>" }\n   - browser_navigate 打开目标站点\n   - browser_get_page_content 获取页面正文 + 带编号（index）可交互元素列表\n   - 需要登录时 pageAgent 会 ask_user 请用户在浏览器面板完成（扫码/验证码），确认后复核\n   - 用 index 定位：browser_type 输入 / browser_click 点击，browser_scroll 翻看视口外内容\n   - browser_get_page_content 提取最终结果并返回摘要\n2. 周期任务：创建 scheduled_task（cron + prompt + 绑定默认助理）\n\n## 示例\n- 每日签到：browser_navigate → browser_get_page_content 找签到入口 → browser_click → browser_get_page_content 确认积分\n- 搜索并提取结果：browser_navigate 搜索页 → browser_type 关键词回车 → browser_get_page_content + browser_scroll 提取结果列表\n\n详见 .claude/skills/web-task-automation/SKILL.md`,
        '用 pageAgent 驱动浏览器完成登录、签到、领积分、搜索、提取内容等网站自动化任务，可配合定时任务每日执行。',
        JSON.stringify(['每日签到', '自动登录', '领取积分', '网站自动化']),
        skillId]);
    } else {
      await driver.run("UPDATE skill SET source = 'builtin' WHERE id = ?", [skillId]);
    }
  }
} catch {}
...
try {
  ...
  const hasSkill = await driver.get('SELECT id FROM skill WHERE id = ?', [skillId]);
  if (!hasSkill) {
    await driver.run('INSERT INTO skill (id, user_id, name, description, triggers_json, body, category, author, enabled, installs, source, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [skillId, 'guest', '即梦每日签到',
    ...
    if (overwriteEnabled) {
      await driver.run("UPDATE skill SET body = ?, description = ?, source = 'builtin' WHERE id = ?", [`# 即梦每日签到领灵感值\n\n自动登录即梦（jimeng.jianying.com）完成每日签到，领取灵感值/积分。配合定时任务可每日自动执行。\n\n## ⚠️ 业务约束（随委派 input 传给 pageAgent）\n- 目标站固定 https://jimeng.jianying.com/，禁止访问 dreamina.ai 等国际版。\n- 登录只走扫码（pageAgent 会 ask_user），禁止代填手机号/验证码。\n- 登录态由 persist:browser-view partition 持久化，首次扫码后复用。\n- 已知稳定选择器：即梦"领积分"入口 #SiderMenuCredit。\n\n## 委派流程\n1. call_agent { agentId: "a_builtin_page_agent", input: "打开 https://jimeng.jianying.com/ 并检查登录状态（有头像=已登录）；未登录则 ask_user 提示用户在浏览器面板扫码登录，用户确认后用 browser_get_page_content 复核；进入'领积分'入口（#SiderMenuCredit），找到'签到/打卡/领灵感'按钮点击签到，用 browser_get_page_content 确认结果并返回摘要。目标站固定 jimeng.jianying.com，禁止访问国际版，禁止代填验证码。" }\n2. 周期任务：scheduled_task cron "0 9 * * *" + prompt "登录即梦签到领灵感值"\n\n详见 .claude/skills/jimeng-daily-checkin/SKILL.md`,
        '每天自动登录即梦（Dreamina，字节跳动 AI 创作平台）并签到领取灵感值/积分。即梦用抖音扫码登录，首次需手动扫码，之后配合定时任务每日自动签到。',
        skillId]);
    } else {
      await driver.run("UPDATE skill SET source = 'builtin' WHERE id = ?", [skillId]);
...
try {
  ...
  const hasSkill = await driver.get('SELECT id FROM skill WHERE id = ?', [skillId]);
  if (!hasSkill) {
    await driver.run('INSERT INTO skill (id, user_id, name, description, triggers_json, body, category, author, enabled, installs, source, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [skillId, 'guest', '本体取数与分析',
      ONTOLOGY_QUERY_DESC,
    ...
    if (overwriteEnabled) {
      await driver.run("UPDATE skill SET body = ?, description = ?, category = '数据分析', source = 'builtin' WHERE id = ?", [ONTOLOGY_QUERY_BODY, ONTOLOGY_QUERY_DESC, skillId]);
    } else {
      await driver.run("UPDATE skill SET source = 'builtin' WHERE id = ?", [skillId]);
    }
  }
...
try {
  ...
  for (const s of builtinSkillDefaults) {
    try {
      const has = await driver.get('SELECT id FROM skill WHERE id = ?', [s.id]);
      if (!has) {
...
try {
  const cols = await driver.all(`SELECT name FROM pragma_table_info('memory')`, []) as Array<{ name: string }>;
  if (!cols.some((c) => c.name === 'type')) {
    await driver.exec(`ALTER TABLE memory ADD COLUMN type TEXT NOT NULL DEFAULT 'agent'`);
  }
  // 使用计数：记忆被注入/检索命中时 +1，供 Dreaming 整理判断"从未使用"的淘汰候选
  if (!cols.some((c) => c.name === 'use_count')) {
    await driver.exec(`ALTER TABLE memory ADD COLUMN use_count INTEGER NOT NULL DEFAULT 0`);
  }
} catch {}
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_memory_user_agent ON memory(user_id, agent_id, last_used_at DESC)'); } catch {}
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_memory_type ON memory(user_id, type, last_used_at DESC)'); } catch {}
...
`);
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_memory_dream_log_user ON memory_dream_log(user_id, started_at DESC)'); } catch {}
...
`);
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_scheduled_task_user ON scheduled_task(user_id, enabled, next_run_at)'); } catch {}

// 定时任务绑定智能体/模型/空间；会话记录来源定时任务（用于列表显示定时标记）
try { await driver.exec('ALTER TABLE scheduled_task ADD COLUMN agent_id TEXT'); } catch {}
try { await driver.exec('ALTER TABLE scheduled_task ADD COLUMN platform_id TEXT'); } catch {}
try { await driver.exec('ALTER TABLE scheduled_task ADD COLUMN model_id TEXT'); } catch {}
try { await driver.exec('ALTER TABLE scheduled_task ADD COLUMN space_id TEXT'); } catch {}
// 定时任务类型：chat=对话式（走 createTask ReAct 循环）；workflow=工作流（走 startWorkflowRun）。bundle 随任务存库，前端关闭也能跑。
try { await driver.exec("ALTER TABLE scheduled_task ADD COLUMN task_type TEXT DEFAULT 'chat'"); } catch {}
try { await driver.exec('ALTER TABLE scheduled_task ADD COLUMN workflow_bundle_json TEXT'); } catch {}
try { await driver.exec('ALTER TABLE scheduled_task ADD COLUMN workflow_inputs_json TEXT'); } catch {}
try { await driver.exec("ALTER TABLE scheduled_task ADD COLUMN workflow_agent_id TEXT"); } catch {}
try { await driver.exec('ALTER TABLE conversation ADD COLUMN scheduled_task_id TEXT'); } catch {}
// 定时任务分组
try { await driver.exec('ALTER TABLE scheduled_task ADD COLUMN group_id TEXT'); } catch {}
// 新调度模型：结构化调度配置 + 有效期（到期自动停用）
try { await driver.exec('ALTER TABLE scheduled_task ADD COLUMN schedule_json TEXT'); } catch {}
try { await driver.exec('ALTER TABLE scheduled_task ADD COLUMN expire_at INTEGER'); } catch {}
await driver.exec(`
...
`);
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_scheduled_task_group_user ON scheduled_task_group(user_id, sort_order)'); } catch {}
...
`);
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_saved_password_user ON saved_password(user_id, host)'); } catch {}
...
`);
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_llm_task_user ON llm_task(user_id, status, updated_at DESC)'); } catch {}
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_llm_task_conv ON llm_task(conversation_id, status)'); } catch {}
...
`);
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_workflow_run_user ON workflow_run(user_id, created_at DESC)'); } catch {}
...
`);
try { await driver.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_data_source_user_name ON data_source(user_id, name)'); } catch {}
...
`);
try { await driver.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_ontology_user_code ON ontology(user_id, code)'); } catch {}
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_ontology_user_ds ON ontology(user_id, datasource_id, status)'); } catch {}
// 旧库迁移：filters_json（查询过滤器，命中才注入 WHERE；与行级策略 policies 不同）
try { await driver.exec("ALTER TABLE ontology ADD COLUMN filters_json TEXT DEFAULT '[]'"); } catch {}
// 旧库迁移：智能体挂载的本体集合（数据查询类智能体按挂载范围取数；'[]' = 不限，可见全部已发布本体）
try { await driver.exec("ALTER TABLE agent ADD COLUMN ontology_ids TEXT DEFAULT '[]'"); } catch {}
...
`);
try { await driver.exec('CREATE INDEX IF NOT EXISTS idx_ontology_group_user_parent ON ontology_group(user_id, parent_id)'); } catch {}
// 旧库迁移：本体所属包
try { await driver.exec('ALTER TABLE ontology ADD COLUMN group_id TEXT'); } catch {}
...
`);
try { await driver.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_std_attr_user_key ON std_attribute(user_id, key)'); } catch {}
// 树结构（P4.7）：kind=group 分组节点（可多级嵌套）/ attr 属性叶子（key/value/def）；parent_id 组装树
try { await driver.exec("ALTER TABLE std_attribute ADD COLUMN parent_id TEXT"); } catch {}
try { await driver.exec("ALTER TABLE std_attribute ADD COLUMN value TEXT"); } catch {}
try { await driver.exec("ALTER TABLE std_attribute ADD COLUMN kind TEXT DEFAULT 'attr'"); } catch {}
// 旧库迁移：selections_json（本体级选择列契约：默认直拼 + 非默认关键字召回）
try { await driver.exec("ALTER TABLE ontology ADD COLUMN selections_json TEXT DEFAULT '[]'"); } catch {}
