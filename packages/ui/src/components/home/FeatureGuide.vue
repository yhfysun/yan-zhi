...
const FEATURE_DETAILS: Record<string, FeatureDetail> = {
  ...
  chat: {
    purpose: '流式任务工作台，支持多任务管理、工具调用可视化、思考链展示、Markdown 渲染与消息蒸馏为 Skill。',
    usage: '在左侧选择或新建任务 → 在底部输入框输入消息 → 点击发送（或回车）即可与模型交流；模型回复支持流式输出、工具调用展开、思考链折叠查看。',
    buttons: [
      '新建任务：左侧顶部按钮创建新任务',
      '消息操作：复制消息内容、将任务蒸馏为可复用 Skill',
      '工具调用展开：点击工具调用块查看入参/出参',
      '思考链展示：折叠/展开模型思考过程',
      '任务切换/重命名/删除：左侧任务列表上下文菜单',
      '模式开关：「+」菜单可开 深度思考 / 计划 / 仅回答 三种模式',
    ],
  },
  'chat-hub': {
    purpose: '统一聊天中心：聚合应用内助手会话、言智节点互聊（Peers）与 IM 渠道（飞书/企业微信/个人微信）消息。',
    usage: '左侧「任务 / 定时任务」列表管理聊天；节点聊天页与其它言智客户端互发消息、互取工具 / Skill / 智能体；配置 IM 连接后，渠道消息自动创建任务跑完并回执。',
    buttons: [
      '任务 / 定时任务：左侧标签页切换',
      '搜索任务：按名称模糊过滤',
      '节点聊天：与互联言智客户端对聊',
