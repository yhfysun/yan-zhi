...
/**
 * Bing Web Search API 后端 —— 纯 HTTP fetch，零浏览器依赖。
 * 使用 Azure Bing Search API v7，需要订阅 key（Ocp-Apim-Subscription-Key header）。
 * 申请：https://www.microsoft.com/en-us/bing/apis/bing-web-search-api
 *
 * 返回结构：{ webPages: { value: [{ name, url, snippet, dateLastCrawled }] } }
 * 对齐豆包/ChatGPT 的搜索 API 路径，不依赖宿主机浏览器。
 */
export class BingApiSearchBackend implements SearchBackend {
  constructor(
    private apiKey: string,
    private endpoint = 'https://api.bing.microsoft.com/v7.0/search',
    private market = 'zh-CN',
  ) {
    if (!apiKey) throw new Error('BingApiSearchBackend 缺少 apiKey（Ocp-Apim-Subscription-Key）');
  }
  ...
  async search(query: string, maxResults: number, timeRange?: TimeRange): Promise<SearchResult[]> {
    ...
    const params = new URLSearchParams({
      q: finalQuery,
      count: String(Math.min(maxResults || 5, 50)),
      mkt: this.market,
      responseFilter: 'Webpages',
    });
    // Bing API 原生支持 freshness 参数：Day/Week/Month（无 Year，用 query 兜底词）
    if (timeRange === 'day') params.set('freshness', 'Day');
    else if (timeRange === 'week') params.set('freshness', 'Week');
    else if (timeRange === 'month') params.set('freshness', 'Month');
    ...
    if (!res.ok) {
      ...
      throw new Error(`Bing API 返回 HTTP ${res.status}${detail ? `: ${detail.slice(0, 200)}` : ''}`);
    }
    const data = await res.json() as { webPages?: { value?: Array<{ name?: string; url?: string; snippet?: string }> } };
    const items = data?.webPages?.value || [];
    return items
      .filter((it): it is { name: string; url: string; snippet: string } => !!it.url && !!it.name)
      .map((it) => ({
        title: String(it.name).slice(0, 300),
        url: String(it.url),
        snippet: String(it.snippet || '').slice(0, 600),
      }));
  }
}
...
export class WebSearchTool implements BuiltInTool {
  name = 'web_search';
  description = '联网搜索信息，返回带标题、URL、摘要的结果列表。支持 timeRange/fresheness 时间过滤（day/week/month/year/recent），用于优先返回最近的资讯/排行/新闻等。建议给推荐、新闻类查询传 timeRange=year，避免返回过期旧数据。sites 是域名白名单（如 ["arxiv.org","*.edu.cn"]），同域名默认最多保留 2 条防止 SEO spam；summarize=true 时由内置 LLM 对结果生成简明摘要（更慢、消耗额外 token，默认关闭）。';
